...
import { Search, Filter, Calendar, MapPin, Users, Award, Loader2 } from 'lucide-react';
...
const ProjectsPage = () => {
  ...
  const allProjects = [
    ...
    // Add more mock projects to demonstrate pagination
    ...Array.from({ length: 21 }, (_, i) => ({
      id: i + 4,
      title: `Layihə ${i + 4}`,
      category: ['residential', 'commercial', 'industrial'][i % 3],
      location: `Bakı, Rayon ${i + 1}`,
      year: '2024',
      area: `${300 + i * 50}m²`,
      status: i % 2 === 0 ? 'Tamamlandı' : 'Davam edir',
      image: '/api/placeholder/400/300',
      description: `Layihə ${i + 4} haqqında qısa məlumat`
    }))
  ];
  ...
  // Function to load projects (simulates API call)
  ...
  //       `/api/projects?page=${page}&limit=${ITEMS_PER_PAGE}&category=${activeFilter}&search=${searchTerm}`
  //     );
      ...
      <div className="container mx-auto px-4">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="px-4 py-2 bg-gold rounded-lg text-main font-medium hover:bg-gold/90 transition-colors hover:scale-105 transform"
                  >
                    Ətraflı
                  </Link>
                </div>
        ...
        {!isLoading && displayedProjects.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-bold text-white mb-2">Nəticə tapılmadı</h3>
            <p className="text-white/60">Axtarış kriteriyalarınızı dəyişdirərək yenidən cəhd edin</p>
          </div>
        )}
        ...
        {isLoading && (
          <div className="text-center py-12">
            ...
            <p className="text-white/80">Layihələr yüklənir...</p>
          </div>
        )}

        {/* Load More Button */}
        {!isLoading && hasMore && displayedProjects.length > 0 && (
          <div className="text-center mt-12">
            <button 
              onClick={handleLoadMore}
              className="px-8 py-4 bg-white/10 backdrop-blur-sm border-2 border-white/20 text-white font-semibold rounded-xl hover:bg-white/20 transition-all hover:scale-105 inline-flex items-center gap-3"
            >
              Daha Çox Yüklə
              <span className="text-xs bg-gold text-main px-2 py-1 rounded-full">
                +{ITEMS_PER_PAGE}
              </span>
            </button>
          </div>
        ...
        {!isLoading && !hasMore && displayedProjects.length > 0 && (
          <div className="text-center mt-12 py-8 border-t border-white/10">
            <p className="text-white/60">
              Bütün layihələr göstərildi ({displayedProjects.length} nəticə)
            </p>
          </div>
        )}
      </div>
